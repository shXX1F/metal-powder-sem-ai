...
output_root=/tos-mlp-zgci/fxx/runs/stat_calibration_20260717/old_stable_grid
...
run_one() {
  ...
  local log_file="/root/code/stat_calibration_20260717/grid_${name}.log"
  mkdir -p "$output_dir"
...
for score in 0.45 0.55 0.65; do
  for mask in 0.40 0.50 0.60; do
    run_one "$score" "$mask" &
  done
done
wait
echo "Threshold grid finished."
