...
output_root=/tos-mlp-zgci/fxx/runs/stat_calibration_20260717/optimization_round3
soup_root=/tos-mlp-zgci/fxx/runs/model_soup_old_round4e42_20260716
weights_a0075="$soup_root/maskrcnn_particle_soup_a0075.pth"
mkdir -p "$output_root"

run_one() {
  local name=$1 weights=$2 score=$3 mask=$4
  local output_dir="$output_root/$name"
  local log_file="/root/code/stat_calibration_20260717/opt3_${name}.log"
  mkdir -p "$output_dir"
...
for score in 0.58 0.62 0.66; do
  for mask in 0.64 0.68 0.72; do
    run_one "a0075_s${score/./}_m${mask/./}" "$weights_a0075" "$score" "$mask" &
  done
done

run_one a0100_s062_m068 "$soup_root/maskrcnn_particle_soup_a0100.pth" 0.62 0.68 &
run_one a0200_s062_m068 "$soup_root/maskrcnn_particle_soup_a0200.pth" 0.62 0.68 &

wait
echo "Optimization round 3 finished."
